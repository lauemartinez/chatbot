...
export class AppComponent implements OnInit {
  ...
  // Main/Init
  ngOnInit() {
    this.createNewMessage('Hola soy Librin! ¿Como puedo ayudarte?', 'sender');
  }
  ...
  async generateSqlResponse(prompt: string) {
    ...
    if(sql_query_array && sql_query_array.length > 1) {
      ...
      this.supabaseService.get(sql_query).then((response) => {
        if(response.data.length < 0){
          this.generateResponse(this.parsePromptFromSqlResponse(response.data));
        } else {
  ...
  private parsePromptFromSqlResponse(data: any){
    var dataString: string[] = [];
    data.forEach((item: any) => dataString.push(JSON.stringify(item)));
    return 'Mostrar de forma entendible esta data: [' + dataString.join(',') + ']';
  }
